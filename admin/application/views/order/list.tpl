    ...
    <table class="table table-bordered table-striped">
        ...
        <{foreach $info['list'] as $i}>
            <tr rel="<{$i['Forder_no']}>">
                ...
                <td class="js-order-status">
                    <{if $i['Forder_status'] eq 1}>初始订单<{elseif $i['Forder_status'] eq 2}>订单取消<{elseif $i['Forder_status'] eq 3}>支付成功<{elseif $i['Forder_status'] eq 4}>内部处理<{elseif $i['Forder_status'] eq 5}>渠道支付失败<{/if}>
                </td>
                <td><{'y-m-d H:i'|date:$i['Fcreate_time']}></td>
                <td><{'y-m-d H:i'|date:$i['Fupdate_time']}></td>
                <td>
                    <{if 'order/orderstatus'|hasPower}>
