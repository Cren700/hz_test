...
CREATE TABLE `t_pay_channel` (
  ...
  `Fout_trade_no` varchar(32) NOT NULL COMMENT '商户系统内部的定单号',
  `Fopenid` varchar(128) NOT NULL COMMENT '用户在商户 appid 下的唯一标识',
  `Ftrade_type` varchar(16) NOT NULL COMMENT 'pay.weixin.jspay',
  `Fpay_result` int(11) NOT NULL COMMENT '支付结果:0—成功;其它—失败',
  `Fpay_info` varchar(64) DEFAULT NULL COMMENT '支付结果信息，支付成功时为空',
  `Ftransaction_id` varchar(32) NOT NULL COMMENT '威富通交易号',
  `Fout_transaction_id` varchar(32) NOT NULL COMMENT '第三方订单号',
  `Ftotal_fee` int(11) NOT NULL COMMENT '总金额',
  `Ffee_type` varchar(8) NOT NULL COMMENT '货币类型，符合 ISO 4217 标准的三位字母代 码，默认人民币:CNY',
  `Fbank_type` varchar(16) NOT NULL COMMENT '银行类型',
  `Fbank_billno` varchar(32) DEFAULT NULL COMMENT '银行订单号，若为微信支付则为空',
  `Ftime_end` varchar(14) NOT NULL COMMENT '支付完成时间，格式为 yyyyMMddhhmmss，如 2009年12月27日9点10分10秒表示为 20091227091010。时区为 GMT+8 beijing。该 时间取自威富通服务器',
  PRIMARY KEY (`Fid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2017-03-10  0:45:09
